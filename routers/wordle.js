...
router.post('/wordle/guess', auth.authenticateToken, async(req, res) => {
    ...
    if (!await Guesses.checkGuessStore(userID, wordlistTitle)) {
        // Create guess
        try {
            const guessStore = new Guesses({wordlistTitle: wordlistTitle, userID: userID})
    ...
    if (wordOfTheDay.word === guess) {
        ...
        won = false
        console.log("When adding saved progress. Need to increment guesses and save the guesses.")
    }
    ...
    if (!await Guesses.addGuess(guess, userID, wordlistTitle, won, colourArray)) {
        return res.status(400).send({error: "Game is already over"})
    }

    return res.status(200).send({success: "word of the day is valid.", won: won, colourArray: colourArray})
})