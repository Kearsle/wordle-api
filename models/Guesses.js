...
guessesSchema.statics.checkGuessStore = async (userID, wordlistTitle) => {
    const guesses = await Guesses.findOne({userID: userID, wordlistTitle: wordlistTitle, expiredAt: { $gt: moment().utc() }})
    if (guesses) {
        return true
    } else {
        return false
...
guessesSchema.statics.addGuess = async (guess, userID, wordlistTitle, won, colourArray) => {
    try {
        const guesses = await Guesses.findOne({userID: userID, wordlistTitle: wordlistTitle, expiredAt: { $gt: moment().utc() }})
        ...
        await Guesses.updateOne(
          { userID: userID, wordlistTitle: wordlistTitle, expiredAt: { $gt: moment().utc() } },
...
guessesSchema.statics.gameInit = async (wordlistTitle, userID) => {
    try {
        ...
        return {guesses: guessesArray, guessesRes: guessesColoursArray, currentGuess: currentGuess, gameOver: gameOver, won: won}
    } catch (error) {
        console.log(error)
        return null
    }
